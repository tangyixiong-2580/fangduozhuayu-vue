<template>
  <div class="details">
    <div
      class="all_details"
      :style="{'background':'linear-gradient(to top, rgba(35, 67, 135, 0.3) 35%, '+collection.bgColor+' 69%) center top / auto no-repeat, url('+'https://images.weserv.nl/?url='+collection.image+') center center / cover no-repeat'}"
    >
      ...
      <footer class="recommend">
        <div class="recommend_content">
          <span class="recommend_number">
            <span class="people_number">{{collection.recommendCount}}</span>
            人推荐了
          ...
          <div class="portrait" style="flex-flow: row-reverse;">
            <span class="avatar" v-for="(p,index) in collection.users" :key="index">
              <span class="root">
                <img alt :src="p.headerImg" class="Head_portrait" />
                {{p}}
              </span>
            </span>
          </div>
    ...
    <div class="item">
      <div class="comment" style="padding: 0px 16px;">
        <div class="comment_item">
          <div class="Flex BookItem" style="margin-left: 16px; border-bottom: none;">
            <aside>
              <div class="book_picture" @click="goBookCollection(b.id,b.bookCollectionId)">
                <img :src="first.book.pic" width="90px" height="128px" />
              </div>
              <article>
                <div class="aa">
                  <h3 class="bb">{{first.book.title}}</h3>
                  <div class="BookImage-content">{{first.book.author}}</div>
                  <div class="BookItem-meta">豆瓣评分 {{first.book.score}}</div>
                </div>
              </article>
            </aside>
          </div>
      ...
      <div class="comment_root">
        <div class="comment_list">
          <p class="full-text">{{first.reason}}</p>

          <footer class="function">
            <div class="flex">
              <div class="character">
                <div class="small">
                  <img :src="'https://images.weserv.nl/?url='+first.user.headerImg" alt class="imgs" />
                </div>
                <div class="Text" style="margin-left: 4px; overflow: hidden;">{{first.user.name}}的推荐</div>
              </div>
              <div class="alignCenter5">
              ...
              <!-- <div class="alignCenter1" v-show="first.isUp==0">
                <div href style="line-height: 0; margin-right: 30px;">
                  <svg
                    @click="give(first.id)"
                    xmlns="http://www.w3.org/2000/svg"
                    width="18"
                    ...
                    viewBox="0 0 19 18"
                  >
                    <g class="icon-i" v-show="first.isUp==1">
                      <path
                  ...
                  <span class="label">{{first.count}}</span>
                </div>
              </div> -->
            </div>
          </footer>
      ...
      <button class="recommendID" @click="recommend()">
        <svg
        ...
        <span style="font-size: 15px; color: rgb(51, 51, 51);">我来推荐一本</span>
      </button>
    </div>
        ...
        <div class="comment_list">
          ...
          <footer class="function">
            <div class="flex">
              <div class="character">
                <div class="small">
                  <img :src="'https://images.weserv.nl/?url='+b.user.headerImg" alt class="imgs" />
                </div>
...
export default {
  ...
  methods: {
    recommend() {
      this.$router.push({
        path: "/Eecommend",
        query: { recommendID: this.listid }
      });
    },
    goBookCollection(id, bookCollectionId) {
      this.$router.push({
        path: "/remark",
        query: { collectionId: id, onebook: bookCollectionId }
      });
      //  this.$router.push({path: "/Comment", query: {collectionId: id}})
      //  this.$router.push({path: "/Comment", query: {onebook: bookCollectionId}})
    },

    goBookColle() {
      this.listid = this.$route.query.booklistid;
      console.info("书单id" + this.listid);
    },
    async getFirst() {
      await axios.get("/collection/detailed/first/" + this.listid).then(res => {
        this.first = res.data.data;
        axios
          .get(
            "/collection/detailed?collectionId=" +
              this.listid +
              "&firstId=" +
              this.first.id
          )
    ...
    //     .get("/collection/detailed?collectionId="+this.listid)
    //     .then(res => res.data.data);
    // },
    async img() {
      console.info("listid:", this.listid);
      this.png = await axios
        .get("/book/collection/img?count=5&collectionId=" + this.listid)
        //.then(rss => res.data.data)
        .then(res => res.data.data);
      console.info("png:", this.png);
    },
    async one() {
    ...
    async give(id) {
      await axios
        .post("/thumbsUp/" + id)
        .then(res => console.info("点赞 " + res.data.data));
    }
